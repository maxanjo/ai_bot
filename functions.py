...
def run_bot(api_key, token):
    ...
    try:
        logger = setup_logger('telegram_bot/logs', 'logs')
        logger.info(f"initialize process for {token}")
        ...
        logger.info(f"Process started id PID {bot_pid}")      
    except Exception as e:
        logger.error(f"An unexpected error occurred while initializing bot: {e}")

def run_bot_script(apiKey, token):
    logger = setup_logger('telegram_bot/logs', token)
    logger.info(f"{token} Bot script initializing")
